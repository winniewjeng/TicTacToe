...
function clicked(row, col) {
    if (gameBoard[row][col] == null) {
        if (!win && !full) { // document.getElementById('box' + boxNum).style.backgroundColor = 'blue';
            if (isPlayer1) {
...
function checkWin() {
    ...
    //win-4 col 0
    else if (gameBoard[0][0] == gameBoard[1][0] && gameBoard[1][0] == gameBoard[2][0] && gameBoard[1][0] != null) {
        if (gameBoard[0][0] == 'X') {
            console.log("Player 1 won!");
        } else {
            console.log("Player 2 won!");
